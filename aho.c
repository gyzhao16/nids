...
int byte_tranform(char* p1, char* p2){
	char* t = (char* )malloc(p2 - p1 + 1);
	memcpy(t, p1, p2 - p1);
	t[p2 -p1] = '\0';
	return atoi(t);
}
...
*aho_get_patterns(const char *pattern_file, int *num_patterns)
{
	...
	*num_patterns = 4;
	...
	//set patterns here
	char dfa_pattern[] = "0 5 71 104 48 115 116 \n\
0 32 147 170 0 138 27 79 71 192 147 170 0 138 27 79 71 192 147 170 0 138 27 79 71 192 147 170 0 138 27 79 71 192 \n\
0 13 33 42 32 83 67 65 78 78 69 82 32 79 78 \n\
0 15 34 41 59 10 13 10 60 47 115 99 114 105 112 116 62 ";
	...
	//get patterns from string above
	i = 0;
	char* delim = "\n";
	char* line = strtok(dfa_pattern, delim);
	while(line != NULL){
		char* delim2 = " ";
		char* dfa_id = strstr(line, delim2);
		patterns[i].dfa_id = *(dfa_id - 1) - 48;
		...
		for(j = 0; j < len; j++){
			p1 = p2;
			p2 = strstr(p2 + 1, delim2);
			int cur_byte = byte_tranform(p1, p2);
