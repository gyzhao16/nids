...
int ids_func(void *ptr)
{
	...
	struct aho_pkt bbatch[BIG_BATCH_SIZE];
...
int main(int argc, char *argv[])
{
 ...
 //           (argc == 1) ? "undefined" : argv[1]);
	...
	//get packet from invoker input

	char* delim = "\"";
	char* p1 = strstr(argv[1], delim);
	char* p2 = strstr(p1 + 1, delim);
	p1 = strstr(p2 + 1, delim);
	p2 = strstr(p1 + 1, delim);
	char* packet = (char* )malloc(p2 - p1);
	memcpy(packet, p1 + 1, p2 - p1 - 1);
	packet[p2 - p1 - 1] = '\0';
	...
	char* delim2 = " ";
	char* dfa_id = strstr(packet, delim2);
	pkts.dfa_id = *(dfa_id - 1) - 48;

	char* dfa_len = strstr(dfa_id + 1, delim2);
	int len = byte_tranform(dfa_id, dfa_len);
	pkts.len = len;

	pkts.content = malloc(len);
	char* p3 = dfa_id;
	char* p4 = dfa_len;
	for(j = 0; j < len; j++){
		p3 = p4;
		p4 = strstr(p4 + 1, delim2);
		int cur_byte = byte_tranform(p3, p4);
		pkts.content[j] = (uint8_t)cur_byte;
		test += cur_byte;
	}

	// pkts.pkt_id = 0;
	// pkts.dfa_id = atoi(argv[0]);

	// int len = atoi(argv[1]);
	// pkts.len = len;
	// pkts.content = malloc(len);
	// for(i = 0; i < len; i++){
	// 	int cur_byte = atoi(argv[i + 2]);
	// 	pkts.content[i] = (uint8_t)cur_byte;
	// }
